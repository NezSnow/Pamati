...
export default function Grainient({
  ...
  useEffect(() => {
    ...
    const startLoop = (
      program: Program,
      mesh: Mesh,
      r: InstanceType<typeof Renderer>,
    ) => {
    ...
    const mount = () => {
      ...
      try {
        ...
        const setSize = () => {
          ...
          const h = Math.max(1, Math.floor(container.getBoundingClientRect().height))
        ...
        sizeObs = new ResizeObserver(setSize)
        sizeObs.observe(container)
        setSize()

        startLoop(program, mesh, renderer)
      } catch (e) {
        console.warn('[Grainient] WebGL2 init failed', e)
      }
    }
    ...
    if (container.offsetWidth > 0 && container.offsetHeight > 0) {
      ...
      wait.observe(container.parentElement ?? container)
